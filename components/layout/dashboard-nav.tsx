    ...
    <nav className={cn("flex flex-col", className)}>
      {routes?.map(({ items, title }, idx) => (
        <React.Fragment key={idx}>
          <span className="mb-1 mt-5 px-4 text-xs font-semibold uppercase text-opacity-90">
            {title}
          </span>
          ...
          {items?.map(({ icon, iconFill, path, title }, idx) => (
            <Link
              className={cn(
                "flex h-10 items-center gap-2 bg-background px-4 font-medium hover:bg-foreground/10",
