...
import EditTechnology from "@/components/technology/edit-form";
...
const Page = ({ params }: Props) => {
  const [data, setData] = useState<{ technology: TechnologyEntity } | null>(
    null,
  );
  ...
  const fetchData = async ({ id }: { id: string }) => {
    ...
    try {
      const res: ApiResponse<{ technology: TechnologyEntity }> = await fetch(
        env.NEXT_PUBLIC_BACKEND_URL + "/api/technology/" + id,
        {
          headers: {
  ...
  return (
    <main className="relative grid min-h-[calc(100svh_-_54px)] w-full gap-4 bg-foreground/5 p-4 md:grid-cols-[320px_1fr] lg:gap-8 lg:p-8">
      {!loading && data ? (
        <EditTechnology defaultData={data.technology} />
      ) : (
