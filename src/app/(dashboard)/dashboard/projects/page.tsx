...
const Page = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["table", "projects"],
    queryFn: async () => {
      const res: ApiResponse<{ rows: ProjectEntity[] }> = await fetch(
        "/api/project",
      ).then((res) => res?.json());
  ...
  const columns: ColumnDef<ProjectEntity>[] = useMemo(
    () => [
      {
          ...
          return (
            <td className="px-4 py-3">
              <div className="flex items-center gap-5">
                <div className="relative grid aspect-square h-12 w-12 place-items-center items-center bg-foreground/5">
                  <Image
          ...
          return (
            <td className="px-4 py-3">
              <div className="grid place-items-center">
                <span>{row.original.showcase ? "TRUE" : "FALSE"}</span>
              </div>
            </td>
          );
        },
