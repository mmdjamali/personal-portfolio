...
type Props = {
  defaultData: ProjectEntity;
};

const EditProject = ({ defaultData }: Props) => {
  const fetch = useCustomFetch();
  const [loading, setLoading] = useState(false);

  const [image, setImage] = useState<string>(defaultData.thumbnail ?? "");

  const [showcase, setShowcase] = useState(defaultData.showcase ?? false);

  const [techs, setTechs] = useState<string[]>(
    defaultData.technologies?.reduce((prev, { id }) => {
      if (id) prev.push(id);
      return prev;
    }, [] as string[]) ?? [],
  );
  ...
  const [github, registerGithub, githubError] = useInputValuePattern({
    defaultValue: defaultData.github ?? "",
    patterns: [
      {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const body: Omit<ProjectEntity, "technologies"> & {
        technologies?: string[];
      } = {} as Omit<ProjectEntity, "technologies"> & {
        technologies?: string[];
      };

      body.id = defaultData.id;
      if (name !== defaultData.name) body.name = name;
      if (image !== defaultData.thumbnail) body.thumbnail = image;
      if (description !== defaultData.description)
        body.description = description;
      if (showcase !== (defaultData.showcase ?? false))
        ...
        <div className="relative flex w-full flex-col gap-5 bg-background p-6">
          <div className="flex w-full items-center justify-between">
            ...
            <span
              className={cn(
                "inline-flex h-4 w-4",
                showcase ? "bg-green-500" : "bg-red-500",
              )}
            />
          </div>
...
const isArrayEqual = (arr1: string[], arr2: string[]) => {
  ...
  const sorted1 = structuredClone(arr1).sort();
  const sorted2 = structuredClone(arr2).sort();

  const bool = sorted1.every((v) => {
    return sorted2.includes(v);
  });
